  ...
  useEffect(() => {
    const fetchOverviewData = async () => {
      try {
        ...
        const sumResponse = await fetch('http://localhost:8000/api/v1/dashboard/summary', {
        ...
        const topProdResponse = await fetch('http://localhost:8000/api/v1/dashboard/top-products', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
        ...
        const invResponse = await fetch('http://localhost:8000/api/v1/inventory/reorder', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (invResponse.ok) {
