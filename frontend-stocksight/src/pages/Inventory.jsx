...
const Inventory = ({ token }) => {
  ...
  const handleAddStock = async (product) => {
    ...
    if (newStock !== null && !isNaN(newStock)) {
      try {
        const response = await fetch(`http://localhost:8000/api/v1/products/${product.originalId}/stock`, {
          method: 'PATCH',
          headers: { 
  ...
  const fetchInventory = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:8000/api/v1/inventory/reorder', {
