...
const Prediction = ({ chartOptions, token }) => {
  ...
  const handleUploadCSV = async (e) => {
    ...
    try {
      ...
      let isDone = false;
      for (let i = 0; i < 15; i++) { 
        const statusRes = await fetch(`http://localhost:8000/api/v1/uploads/${uploadData.id}`, { headers: { 'Authorization': `Bearer ${token}` } });
      ...
      const prodRes = await fetch('http://localhost:8000/api/v1/products', { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleRunPrediction = async (e) => {
    ...
    try {
      const configRes = await fetch('http://localhost:8000/api/v1/config', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const forecastRes = await fetch('http://localhost:8000/api/v1/forecasts', {
        method: 'POST',
        headers: {
      ...
      let isDone = false;
      for (let i = 0; i < 20; i++) {
        const statusRes = await fetch(`http://localhost:8000/api/v1/forecasts/${forecastData.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const statusData = await statusRes.json();
      ...
      const detailRes = await fetch(`http://localhost:8000/api/v1/forecasts/${forecastData.id}/details`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
