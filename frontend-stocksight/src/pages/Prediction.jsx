...
const Prediction = ({ chartOptions, token }) => {
  ...
  useEffect(() => {
    ...
    const fetchInitialProducts = async () => {
      try {
        const response = await fetch('stocksightcapstone-production.up.railway.app/api/v1/products', {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (response.ok) {
          const data = await response.json();
  ...
  const handleUploadCSV = async (e) => {
    ...
    try {
      const uploadRes = await fetch('stocksightcapstone-production.up.railway.app/api/v1/uploads', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formData
      });
      ...
      let isDone = false;
      for (let i = 0; i < 15; i++) { 
        const statusRes = await fetch(`stocksightcapstone-production.up.railway.app/api/v1/uploads/${uploadData.id}`, { headers: { 'Authorization': `Bearer ${token}` } });
      ...
      const prodRes = await fetch('stocksightcapstone-production.up.railway.app/api/v1/products', { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleRunPrediction = async (e) => {
    ...
    try {
      const configRes = await fetch('stocksightcapstone-production.up.railway.app/api/v1/config', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const forecastRes = await fetch('stocksightcapstone-production.up.railway.app/api/v1/forecasts', {
        method: 'POST',
        headers: {
      ...
      let isDone = false;
      for (let i = 0; i < 20; i++) {
        const statusRes = await fetch(`stocksightcapstone-production.up.railway.app/api/v1/forecasts/${forecastData.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const statusData = await statusRes.json();
      ...
      const detailRes = await fetch(`stocksightcapstone-production.up.railway.app/api/v1/forecasts/${forecastData.id}/details`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
